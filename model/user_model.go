...
func GetUserByUsername(user *entity.User, username string, client *statsd.Client) (err error) {
	t := client.NewTiming()
...
func UpdateUserWithSamePwd(user *entity.User, id string, client *statsd.Client) (err error) {
	t := client.NewTiming()
	if !tool.CheckUsername(user.Username) {
...
func UpdateUserWithDiffPwd(user *entity.User, id string, client *statsd.Client) (err error) {
	t := client.NewTiming()
	if !tool.CheckUsername(user.Username) {
		return errors.New("update user error: username is not email address!")
	}
